...
hiveq <- function(d, BirthRate, b, Sf) {
  write.csv(d,
            file = paste0(deparse(BirthRate),".",deparse(b),".",deparse(Sf),".",".FinalFigure3FiftyFiftyModelNoPetrurbincode.csv"),
            row.names = FALSE)
}
...
set.seed(Reps)
b =  as.numeric(args[2])  #Increase In Summer Growth Rate from Helpers (Usually = 0.1, b=0.075 in Supp Figure 6)
BirthRate = 0.05 #Basal birth rate
Sf = 0.1 #Proportion of Foundresses that Die before end of summer 
Sm = as.numeric(args[3])  #Mort Rate of Males (Usually = 0.4, Sm = 1.8 in Supp Figure 6)
DIY <- as.numeric(args[4]) #Length from diapause > diapause (120 days in Figure 3a, SuppFigure 6a; 135 days in Figure 3b, Supp Figure 6b, 150 days in Figure 3c, Supp Figure 4, Supp Figure 6c, 180 Days in Figure 3d, Supp Figure 6d, 154 days in Supp Figure 5)
