        ...
        #This Function will convert set of nests F1 to a new generation
        
        Perturb <- function(F1, Perturbation, Percentage){
        Intruders <- floor(length(F1)*Percentage)
        for(i in 1:Intruders){
        F1[[i]][["Female"]][["Starting Hel"]][1] <- F1[[i]][["Female"]][["Starting Hel"]][1] + Perturbation
        F1[[i]][["Female"]][["Starting Hel"]][2] <- F1[[i]][["Female"]][["Starting Hel"]][2] + Perturbation
        F1[[i]][["Male"]][["Starting Hel"]] <- F1[[i]][["Male"]][["Starting Hel"]] + Perturbation
        }
        return(F1)
        }
        ...
        Sfe = Sf / DIY #Per Day Mort Rate
        Sme = Sm / DIY #Per Day Mort Rate
        ...
        for(i in c(1:N1)){
        F1[[i]] <- Nest1
        F1[[i]][[4]] <- sample((RepAge):(RepAge), 1) #Starting Female
        }
        for(i in c(N2:N)){
        F1[[i]] <- Nest1
        F1[[i]][[4]] <- sample((RepAge):(RepAge), 1) #Starting Female
        ...
        for(i in 1:x){
        F2[[i]] <- Nest1
        F2[[i]][[4]] <- sample((RepAge):(RepAge), 1) #ImmatureFemales
        }
        F3 <- list()
        x <- Max*DIY*N
        for(i in 1:x){
        F3[[i]] <- Nest1
        F3[[i]][[4]] <- sample((RepAge):(RepAge), 1) #MatureFemales
        }
        M1 <- list()
        x <- Max*DIY*N
        for(i in 1:x){
        M1[[i]] <- Male1
        M1[[i]][[5]] <- sample((RepAge):(RepAge), 1) #ImmatureMales
        }
        M2 <- list()
        x <- Max*DIY*N
        for(i in 1:x){
        M2[[i]] <- Male1
        M2[[i]][[5]] <- sample((RepAge):(RepAge), 1) #MatureMales
        }
        Gen=1
        means = c(0, 0, 0)
        sds = c(0, 0, 0)
...
df <- list(NextGeneration(F1), NextGeneration(F1), NextGeneration(F1), NextGeneration(F1), NextGeneration(F1), NextGeneration(F1), NextGeneration(F1), NextGeneration(F1), NextGeneration(F1), NextGeneration(F1))
means = data.frame(aaply(laply(df, as.matrix), c(2, 3), mean))
sd = data.frame(aaply(laply(df, as.matrix), c(2, 3), sd))
...
for(i in 1:End){
  if(i>=TauM){
    ...
    Ph = min(max(Ph,0),1)
    Pr = 1-Pm-Ph
    df$Male[i+1] = ((1-GammaM)*df$Male[i]) + (((1-GammaM)^TauM)*((Alpha*df$Repr[i+1-TauM] + Beta*(df$Help[i+1-TauM])))*Pm)
    df$Help[i+1] = (df$Help[i]) + (((Alpha*df$Repr[i+1-TauH] + Beta*(df$Help[i+1-TauH])))*Ph)
  }
  else{
    df$Male[i+1] = df$Male[i]
    df$Help[i+1] = df$Help[i]
  }
  if(i>=TauR){
    ...
    print(Ph)
    Pr = 1-Pm-Ph
    df$Repr[i+1] = ((1-GammaR)*df$Repr[i]) + (((1-GammaR)^TauR)*((Alpha*df$Repr[i+1-TauR] + Beta*(df$Help[i+1-TauR])))*Pr)
  }
  else{
    df$Repr[i+1] = df$Repr[i]
  }
}
df$Repr <- df$Repr-100
myd <- melt(df, id="Day")
...
cols <- c("Male" = "#88cde9", "Help" = "#74e841", "Repr"="#cb49e3")
...
p + xlab("Day") + ylab("Number of Offspring") + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position = 'none', axis.text=element_text(size=14),axis.title=element_text(size=20))
